...
int main(){
    ...
    while(1){
        ...
        AngleResults results = model.getAngles(tensor);
        for (const auto &angle : results) {
            if(angle.index == AngleType::ANGLE_0) {
                std::cout << "Detected Angle: 0, Score: " << angle.score << std::endl;
            } else if(angle.index == AngleType::ANGLE_180) {
                std::cout << "Detected Angle: 180, Score: " << angle.score << std::endl;
            }
        }
    }
}